  ...
  return (
    <div className="">
      ...
      <div className="flex flex-col py-12 gap-8 items-center text-2xl ">
        ...
        <p>
          Order online for pickup or{" "}
          <Link href="#market-info" className="text-(--rust)">
            visit our stand
          </Link>
        </p>
        <Link
          href="/shop"
          className="text-white bg-(--teal) box-border border border-transparent hover:bg-(--teal-hover) font-base leading-5 rounded-md text-lg px-4 py-2"
        >
          ...
          <div className="w-full">
            {loading ? (
              ...
              <div className="text-sm text-[var(--input-border)] text-center">
                <p className="pb-2">There are no posts at the moment.</p>
                <p>
                  Check back soon to see updates on what we're growing and get
                  the scoop on new deals!
                </p>
              </div>
            ) : (
              <div className="flex h-64 shadow-lg shadow-(--rust) border border-(--footer) rounded-xl p-8 text-left gap-6">
                ...
                <div className="flex flex-col w-3/4">
                  ...
                  <p className="mb-6">
                    posted {formatDate(posts[postIndex].created_at)}
                  </p>
                  <p className="mb-6 line-clamp-2">
                    {posts[postIndex].excerpt}
                  </p>
                  <button
                    onClick={() =>
                      router.push(
                        `/blog/${posts[postIndex].id}?${new URLSearchParams(window.location.search)}`,
                      )
                    }
      ...
      <div id="market-info" className="relative bg-(--secondary) py-12">
        ...
        <div className="relative flex flex-col md:flex-row max-w-6xl mx-auto mb-12 justify-center md:gap-10">
          <div className="relative basis-1/2 w-full max-w-lg h-64">
            <Image
              src="/stand_photo.jpeg"
              alt="photo of farmer's market stand"
              fill
              className="object-cover rounded-sm"
            />
          </div>
          <div className="relative flex flex-col basis-1/2 px-6 justify-between">
            ...
            <p className="text-(--input-border) text-sm">
              *Check our socials for updates on stand schedule
            </p>
            <Link
              href="/about"
              className="w-fit text-white bg-(--rust) box-border border border-transparent hover:bg-(--dark-rust) font-base leading-5 rounded-md text-sm px-4 py-2"
            >
              Learn More
            </Link>
          </div>
